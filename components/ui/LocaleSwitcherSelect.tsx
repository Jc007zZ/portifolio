...
export function LocaleSwitcherSelect() {
  ...
  function onChange(value: string) {
   ...
   function OpenLanguage(){
    if (buttonRef1.current && buttonRef2.current) {
      if (buttonRef1.current.classList.contains('hidden') && buttonRef2.current.classList.contains('hidden')){
  ...
  return (
    <div className="size-6">
      <LanguageIcon onClick={OpenLanguage} className='hover: cursor-pointer' />
      <button ref={buttonRef1} className='hidden' onClick={() => { onChange('en'); OpenLanguage()}}>English</button>
      <button ref={buttonRef2} className='hidden' onClick={() => { onChange('pt'); OpenLanguage()}}>Português</button>
    </div>
  );
}
;